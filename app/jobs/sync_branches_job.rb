class SyncBranchesJob < ApplicationJob
  ...
  def perform(*project_ids)
    unless project_ids.present?
      ...
      Project.each {|prj|
        SyncBranchesJob.perform_later(prj.id.to_s) if prj.code_manager.present? && prj.config.present?
      }
      return
    end
  ...
  def sync_branches(prj)
    ...
    branches = resp.map {|d|
      ...
      branch.build.rerun if branch.commit_changed? && branch.build.present? && branch.build.name != prj.config[:RELEASE][:BUILD][:NAME]
